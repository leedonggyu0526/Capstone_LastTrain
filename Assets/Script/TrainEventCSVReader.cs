...
public class TrainEventCSVReader
{
    ...
    private Sprite LoadEventImage(string eventID)
    {
        ...
        string[] possiblePaths = {
            $"Assets_leedonggyu/Sprites/EventImages/{eventID}",
            $"Assets_leedonggyu/Sprites/EventImages/{eventID.ToLower()}",
            $"Assets_leedonggyu/Sprites/EventImages/{eventID.ToUpper()}",
            $"EventImages/{eventID}",
            $"Images/{eventID}",
            $"Sprites/{eventID}"
        };
        ...
        try
        {
            ...
            else
            {
                ...
                Debug.LogWarning($"  1. 이미지 파일이 'Assets/Resources/Assets_leedonggyu/Sprites/EventImages/{eventID}.png' 경로에 있는지 확인");
